...
M.pixels = function(x, y) return {x / 32, y / 32} end

M.rotate = function(dir, ang) return (dir + ang) % 8 end

M.increase = function(mx, k, x) mx[k] = (mx[k] or 0) + x end
...
M.iconWithColor = function(color, icon, size, scale)
    return {
        ...
        {icon = icon, icon_size = size or 64, scale = scale or 0.375},
    }
end
...
M.filter = function(xs, f)
    ...
    for i = n, len do xs[i] = nil end
end

M.any = function(mx, f)
    for k, v in pairs(mx) do if f(k, v) then return true end end
...
M.merge = function(mx, my)
    ...
    for k, v in pairs(mx) do mz[k] = v end

    for k, v in pairs(my) do mz[k] = v end
