    ...
    "show-train-stations", function(event)
        ...
        if center["train-stations-frame"] == nil then
            ...
            for sid, station in pairs(global.trainStations) do
                local name = util.any(
                    station.stops, function(_, stop)
                        return not stop.get_or_create_control_behavior()
                                   .disabled
                    end
                ) and station.name or "*** " .. station.name
...
script.on_event(
    defines.events.on_runtime_mod_setting_changed, function(event)
        if event.setting == "ats-update-interval" then
            ...
            script.on_nth_tick(
                settings.global["ats-update-interval"].value, updateAll
            )
        end
    end
)
